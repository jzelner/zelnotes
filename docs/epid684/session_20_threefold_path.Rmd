...
>- Quick overview of thursday in-class workshop.
...
<span class="alert">npooled approach:</span> U Fit a separate model to each unit ($j$), assuming outcomes in each unit are independent:
...
- $y_{ij} = \alpha + \beta x_i + \epsilon_{i} + \epsilon_{j}$

<div class="fragment">Allow <span class="alert">effects</span> to vary across clusters, but constrain them to come from the same distribution:

- $\epsilon_{i} \sim N(0, \sigma_{i}^2)$

- $\epsilon_{j} \sim N(0, \sigma_{j}^2)$</div>
...
<img src="../images/gelman_county_uranium.png">

# Partial pooling most benefits places with <span class="alert">less data</span> {.imgslide}

<img src="../images/gelman_basement_effect.png">
