...
ggsave("output/moran/checkerboard.pdf")

g2 <- moran_grid_plot(df,queen=TRUE)
ggsave("output/moran/checkerboard_queen.pdf")

df2 <- df
df2$z <- sort(df2$z)
g2 <- moran_grid_plot(df2)
ggsave("output/moran/clustered.pdf")

df3 <- df
df3$z <- sample(df2$z)
g3 <- moran_grid_plot(df3)
ggsave("output/moran/random.pdf")

df3 <- df
df3$z <- sample(df2$z)
g3 <- moran_grid_plot(df3)
ggsave("output/moran/random_2.pdf")
