...
```{r}
g <- ggplot(minnesota_radon) + 
  geom_sf(aes(fill = median_radon )) + 
  scale_fill_viridis_c() +
  ggtitle(paste0("Median household radon by MN county (I=", round(moranFromSF(minnesota_radon$median_radon, minnesota_radon),2), ")"))
plot(g)
```
...
g <- ggplot(random_minnesota) + 
  geom_sf(aes(fill = median_radon )) + 
  scale_fill_viridis_c() +
  ggtitle(paste0("Spatially randomized median radon by MN county (I=", round(moranFromSF(random_minnesota$median_radon, random_minnesota),2), ")"))
plot(g)
```
...
```{r}
g <- ggplot() + 
  geom_histogram(aes(x=csr_dist$midist)) + 
